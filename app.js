...
app.get("/SoulaymaneMap", (req, res) => {
  let sql = "SELECT * FROM monument";
  let query = connection.query(sql, (err, rows) => {
    if (err) throw err;
    res.render("SoulaymaneMap", {
      title: "gestion MAPS",
      donnes: rows,
    });
  });
...
app.post("/monuments/save", (req, res) => {
  ...
  if (
    teswira.mimetype == "image/jpeg" ||
    teswira.mimetype == "image/png" ||
    teswira.mimetype == "image/gif"
  ) {
